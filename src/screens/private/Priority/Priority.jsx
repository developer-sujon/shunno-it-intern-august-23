...
import { usePriorityDeleteMutation, usePriorityListQuery } from '../../../redux/services/priorityService';
...
const Priority = () => {
  ...
  const { data, isLoading } = usePriorityListQuery();

  const [priorityDelete] = usePriorityDeleteMutation();

  const deleteItem = (id) => {
    AleartMessage.Delete(id, priorityDelete);
  };
    ...
    },
    {
      ...
      accessor: (d) => (
        <div className="bodySmall">
          ...
          <span className="ms-1"> {d?.value}</span>
        </div>
      ),
      sort: true,
    },
    ...
    },
    {
      ...
      accessor: (d) => (
        <div className="bodySmall">
          ...
          >
            <Link to={`/priority-create-update?id=${d?.id}`}>
          ...
          <Card.Body>
            <Row>
              <Col className="d-flex justify-content-between p-2" sm={12}>
                <h5>{t('priorities')}</h5>
                <Link to={'/priority-create-update'}>
