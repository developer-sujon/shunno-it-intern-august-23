...
import { useLazyTicketDetailsQuery, useTicketReplyMutation } from '../../../redux/services/ticketService';
...
const CustomerTicket = () => {
  ...
  const [trigger, { data: ticketDetails, isSuccess: ticketDetailsSuccess }] = useLazyTicketDetailsQuery();
  const [ticketReply, { data: replyData, isLoading: replyLoading, isSuccess: replySuccess }] = useTicketReplyMutation();
  const {
    control,
    handleSubmit,
    reset,
    ...
    resolver: yupResolver(
      yup.object({
        description: yup.string().required().required(t('description is required')),
      })
    ),
  });
  ...
  useEffect(() => {
    if (tokenDetails) {
      ...
      const response = async () => await trigger({ ticketID: tokenDetails?.ticketID, shunnoID: tokenDetails?.shunnoID });
      response();
    }
  ...
  useEffect(() => {
    if (token) {
      try {
        ...
        console.log(error);
        navigate(`/support-ticket/create?shunnoID=${shunnoID}&clientApp=${clientApp}`);
      }
    }
  }, []);
                  ...
                  <Card>
                    <Card.Body>
                      <Card.Title className="mb-3">
                        {t('reply ticket')} : <span className="text-primary">{singleTicket?.ticketID}</span>
                        &nbsp;&nbsp;
                        ...
                        <Row>
                          <Col sm={4}>
                            <Button size="sm" className="mt-2" type="submit">
                              {replyLoading ? <Spinner size="sm" color="light" /> : t('reply ticket')}
                            </Button>
                          </Col>
                        </Row>
                      </Form>
